...
enum class ParseState { Global, InDetector, InSource };
...
public:
  BaseExperimentMessenger() {
    ...
    fMatCreateX = builder.Params("/exp/material/createx", Param("mat", 's'),
                                 Param("x", 'd'));
    fMatAddMass =
    ...
    fDecayIon = builder.Params("/exp/source/decay/ion", Param("Z", 'd'),
                               Param("A", 'd'));

    fDecayExcitation =
        builder.DoubleUnit("/exp/source/decay/excitation", "Energy");

    fDecayCharge =
        builder.Params("/exp/source/decay/charge", Param("charge", 'd'));
    ...
    fSrcLayerType = builder.String("/exp/SourceLayer/type");
  ...
  void SetNewValue(Geant4::G4UIcommand *cmd, Geant4::G4String value) override {
    ...
    if (cmd == applyCommand) {
      if (fState != ParseState::Global) {
        Geant4::G4Exception("Messenger", "ApplyError",
                            Geant4::G4ExceptionSeverity::JustWarning,
                            "Calling /exp/apply while a block is still open. "
                            "Proceeding anyway, but data might be incomplete.");
      }
      auto cfg =
          std::shared_ptr<const BetaSimLib::Models::BaseExperimentConfig>(
              std::move(fCfg));

      EventDispatcher::EventDispatcher::GetExperimentConfigReadyEvent().Invoke(
          cfg);
    ...
    if (cmd == fDetStart) {
      if (fState != ParseState::Global) {
        Geant4::G4Exception("Messenger", "StateError",
                            Geant4::G4ExceptionSeverity::FatalException,
                            "Cannot start DetectorLayer inside another block.");
      }
      fState = ParseState::InDetector;
      fTempDetector =
          BetaSimLib::Models::DetectorConfig(); // Очищаем временный буфер
      return;
    }
    ...
    if (cmd == fDetEnd) {
      if (fState != ParseState::InDetector) {
        Geant4::G4Exception("Messenger", "StateError",
                            Geant4::G4ExceptionSeverity::FatalException,
                            "Mismatched /exp/DetectorLayer/end.");
      }
      fState = ParseState::Global;
      GetConfigInstance()->detector =
          fTempDetector; // Сохраняем в основной конфиг
      return;
    }
    ...
    if (cmd == fSrcLayerStart) {
      if (fState != ParseState::Global) {
        Geant4::G4Exception("Messenger", "StateError",
                            Geant4::G4ExceptionSeverity::FatalException,
                            "Cannot start SourceLayer inside another block.");
      }
    ...
    if (cmd == fSrcLayerEnd) {
      if (fState != ParseState::InSource) {
        Geant4::G4Exception("Messenger", "StateError",
                            Geant4::G4ExceptionSeverity::FatalException,
                            "Mismatched /exp/SourceLayer/end.");
      }
      fState = ParseState::Global;
    ...
    if (cmd == fStackXY) {
      ...
      if (fState == ParseState::InDetector) {
        ...
        Geant4::G4Exception("Messenger", "ContextWarning",
                            Geant4::G4ExceptionSeverity::JustWarning,
                            "/exp/stack/xy used globally. Ignoring.");
      }
      return;
    }
    ...
    if (cmd == fLayerAdd) {
      ...
      if (fState == ParseState::InDetector) {
        ...
        Geant4::G4Exception("Messenger", "ContextWarning",
                            Geant4::G4ExceptionSeverity::JustWarning,
                            "/exp/layer/add used globally. Ignoring.");
      }
      return;
    }
    ...
    // --------------------------------------------------------
    if (cmd == fSourceType) {
      GetConfigInstance()->sourceType =
          (value == "gun") ? BetaSimLib::Models::SourceType::Gun
    ...
    if (cmd == fGunDir) {
      ...
      GetConfigInstance()->gun.dir =
          Geant4::G4ThreeVector(std::stod(dx), std::stod(dy), std::stod(dz))
              .unit();
      return;
    }
    ...
    if (cmd == fDecayIon) {
      ...
      GetConfigInstance()->isotope.z = static_cast<int>(std::stod(z));
      GetConfigInstance()->isotope.a = static_cast<int>(std::stod(a));

      return;
    }

    if (cmd == fDecayExcitation) {
      GetConfigInstance()->isotope.excitationEnergy =
          fDecayExcitation->GetNewDoubleValue(value);
    ...
    if (cmd == fDecayCharge) {
      ...
      GetConfigInstance()->isotope.ionCharge = std::stod(charge);
...
private:
  ...
  // Радиоизотопный источник
  Geant4::G4UIcommand *fDecayIon = nullptr;
  Geant4::G4UIcmdWithADoubleAndUnit *fDecayExcitation = nullptr;
  Geant4::G4UIcommand *fDecayCharge = nullptr;
#pragma endregion
};
} // namespace BetaSimLib::Messenger