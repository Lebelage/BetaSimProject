...
public:
  void BuildForMaster() const override {
    SetUserAction(new BetaSimLib::Actions::RunAction(manageVisualization));
  }

  void Build() const override {
    // Worker threads не должны трогать /vis.
    SetUserAction(new BetaSimLib::Actions::RunAction(false));
...
private:
  void InitializeRunManager(int argc, char **argv) {
    ...
    Geant4::cout() << "[Runner] Thread count = " << threadCount << Geant4::endl;
    ...
    runManager->SetUserInitialization(new UserActionInitialization(
        experimentState, genManager, manageVisualization));
...
private:
  void OnExperimentUpdated(
    ...
    Geant4::cout() << "[Runner] Geometry update scheduled. "
