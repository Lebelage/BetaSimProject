...
private:
  ...
  ~ExtendedMaterialService() { ClearCache(); };
#pragma endregion
...
public:
  ...
  std::optional<ExtendedG4Material *> Get(const Geant4::G4String &name) {
    ...
    if (key == "Ni63_Source") {
      auto *isoNi63 = new Geant4::G4Isotope("Ni63", 28, 63,
                                            62.9296 * Geant4::g / Geant4::mole);
      auto *elNi63 = new Geant4::G4Element("RadioactiveNi", "Ni", 1);
      elNi63->AddIsotope(isoNi63, 100.0 * Geant4::perCent);

      auto *ni63Mat = new Geant4::G4Material(
          "Ni63_Material", 8.908 * Geant4::g / Geant4::cm3, 1);
    ...
    if (name.rfind("G4_", 0) == 0) {
      ...
      if (!m)
        Geant4::G4Exception("Materials", "NoNIST",
    ...
    if (!fCfg) {
      Geant4::G4Exception(
          "Materials", "NoConfig", Geant4::G4ExceptionSeverity::FatalException,
    ...
    Geant4::G4Exception("Materials", "UnknownMaterial",
                        Geant4::G4ExceptionSeverity::FatalException,
...
private:
  ExtendedG4Material *BuildFromSpec(const std::string &name,
    ...
    if (!spec.finalized) {
      Geant4::G4Exception("Materials", "NotFinalized",
                          Geant4::G4ExceptionSeverity::FatalException,
    ...
    if (name.find("AlGaN") != std::string::npos ||
        name.find("algan") != std::string::npos) {
      double m_total = x * MaterialsConstants::M_Al +
                       (1.0 - x) * MaterialsConstants::M_Ga +
                       MaterialsConstants::M_N;
      density = x * MaterialsConstants::DENS_AlN +
                (1.0 - x) * MaterialsConstants::DENS_GaN;

      components.push_back({"Al", (x * MaterialsConstants::M_Al) / m_total});
      components.push_back(
          {"Ga", ((1.0 - x) * MaterialsConstants::M_Ga) / m_total});
      components.push_back({"N", MaterialsConstants::M_N / m_total});
    } else if (name.find("InGaN") != std::string::npos ||
               name.find("ingan") != std::string::npos) {
      double m_total = x * MaterialsConstants::M_In +
                       (1.0 - x) * MaterialsConstants::M_Ga +
                       MaterialsConstants::M_N;
      density = x * MaterialsConstants::DENS_InN +
                (1.0 - x) * MaterialsConstants::DENS_GaN;
      ...
      Geant4::G4Exception("Materials", "UnknownAlloy",
                          Geant4::G4ExceptionSeverity::FatalException,
                          ("Cannot build alloy from x for: " + name +
                           ". Name must contain AlGaN or InGaN.")
                              .c_str());
    }

    // ИСПРАВЛЕНО: Префиксы Geant4::
    Geant4::G4Material *mat = new Geant4::G4Material(
        name, density * (Geant4::g / Geant4::cm3), components.size());
    ...
    for (const auto &[elName, fraction] : components) {
      ...
      if (!el) {
        Geant4::G4Exception("Materials", "NoElement",
                            Geant4::G4ExceptionSeverity::FatalException,
  ...
  ExtendedG4Material *BuildFromSpec(const std::string &name,
    ...
    if (spec.density <= 0) {
      Geant4::G4Exception("Materials", "BadDensity",
                          Geant4::G4ExceptionSeverity::FatalException,
    ...
    if (spec.useAtoms) {
      ...
      Geant4::G4Material *mat =
          new Geant4::G4Material(name, spec.density, (int)spec.mass.size());
  ...
  float CalculateEg(std::string matName, float x) {
    ...
    if (matName.find("AlGaN") != std::string::npos ||
        matName.find("algan") != std::string::npos) {
      return x * MaterialsConstants::EG_ALN +
             (1.0f - x) * MaterialsConstants::EG_GAN -
             MaterialsConstants::B_ALGAN * x * (1.0f - x);
    } else if (matName.find("InGaN") != std::string::npos ||
               matName.find("ingan") != std::string::npos) {
      return x * MaterialsConstants::EG_INN +
             (1.0f - x) * MaterialsConstants::EG_GAN -
             MaterialsConstants::B_INGAN * x * (1.0f - x);
    } else if (matName.find("GaN") != std::string::npos ||
               matName.find("gan") != std::string::npos) {
      return MaterialsConstants::EG_GAN;
    }

    std::cerr << "[ОШИБКА] Неизвестный материал для расчета Eg: " << matName
              << std::endl;
    return 0.0f;
  }
#pragma endregion
...
} // namespace BetaSimLib::Materials